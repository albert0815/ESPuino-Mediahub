"""Reads a plain podcast RSS feed (concept §7.4).

The second source behind a "Podcast" card, next to the ARD Sounds
catalogue: the admin pastes any podcast's feed URL and the hub takes the
episodes from there. No ARD involvement at all — no unofficial API, no
catalogue — which makes this the more durable of the two sources, and the
only one that works for podcasts outside ARD.
...
def feed_key(feed_url):
    ...
    Plays the role ARD's numeric show id plays for catalogue cards, so
    podcast_cache.episode_relpath works unchanged for both sources.
    """
